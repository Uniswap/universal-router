...
library UniswapV2Library {
        ...
        returns (address pair, address token0)
    {
        ...
        return (pairForPreSorted(factory, token0, token1), token0);
    }
    ...
    {
        (address token0, address token1) = UniswapPoolHelper.sortTokens(tokenA, tokenB);
        (uint256 reserve0, uint256 reserve1,) = IUniswapV2Pair(pairForPreSorted(factory, token0, token1)).getReserves();
