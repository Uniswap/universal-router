...
contract V2SwapRouter {
    ...
    function _v2Swap(address[] memory path, address recipient) private {
        unchecked {
            ...
            (address pair, address token0) =
                V2UniswapLibrary.pairAndToken0For(V2_FACTORY, PAIR_INIT_CODE_HASH, path[0], path[1]);
            for (uint256 i; i < path.length - 1; i++) {
                ...
                uint256 amountOutput = V2UniswapLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
                (uint256 amount0Out, uint256 amount1Out) =
                    input == token0 ? (uint256(0), amountOutput) : (amountOutput, uint256(0));
                address nextPair;
                (nextPair, token0) = i < path.length - 2
                    ? V2UniswapLibrary.pairAndToken0For(V2_FACTORY, PAIR_INIT_CODE_HASH, output, path[i + 2])
                    : (recipient, address(0));
    ...
    {
        address pair;
        (amountIn, pair) = V2UniswapLibrary.getAmountInMultihop(V2_FACTORY, PAIR_INIT_CODE_HASH, amountOut, path);
