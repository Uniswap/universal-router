...
interface IUniversalRouter {
    ...
    /// permissionless function that reaches this entrypoint can have its own capital drained. The credit
    /// direction is equally unsafe: positive delta left under the router by one call is not reserved for
    /// whoever created it, so any contract that gains control before the next call -- an attacker's hook or
    /// callback -- can opt itself in here and `TAKE` it. Keep plans delta-neutral across call boundaries in
    /// both directions, and never pass control to an untrusted contract while a router delta is open. If a
    /// function does not open its own unlock, call `execute` instead, which refuses to run nested.
