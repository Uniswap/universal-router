...
abstract contract Dispatcher is Payments, V2SwapRouter, V3SwapRouter, Callbacks, LockAndMsgSender {
    ...
    function dispatch(bytes1 commandType, bytes calldata inputs) internal returns (bool success, bytes memory output) {
        ...
        if (command < Commands.FOURTH_IF_BOUNDARY) {
            if (command < Commands.SECOND_IF_BOUNDARY) {
                ...
                if (command < Commands.FIRST_IF_BOUNDARY) {
                    if (command == Commands.V3_SWAP_EXACT_IN) {
                        ...
                        address recipient = inputs.getAddress();
                        uint256 amountIn = inputs.getUint256(0x20);
                        uint256 amountOutMin = inputs.getUint256(0x40);
                        // 0x60 offset is the path, decoded below
                        bytes calldata path = inputs.toBytes(3);
                        bool payerIsUser = inputs.getBool(0x80);
                        address payer = payerIsUser ? lockedBy : address(this);
                        v3SwapExactInput(map(recipient), amountIn, amountOutMin, path, payer);
                    } else if (command == Commands.V3_SWAP_EXACT_OUT) {
                        // equivalent: abi.decode(inputs, (address, uint256, uint256, bytes, bool))
                        address recipient = inputs.getAddress();
                        uint256 amountOut = inputs.getUint256(0x20);
                        uint256 amountInMax = inputs.getUint256(0x40);
                        bytes calldata path = inputs.toBytes(3);
                        // 0x60 offset is the path, decoded below
                        bool payerIsUser = inputs.getBool(0x80);
                        address payer = payerIsUser ? lockedBy : address(this);
                        ...
                        address token = inputs.getAddress();
                        address recipient = inputs.getAddress(0x20);
                        uint160 amount = inputs.getUint160(0x40);
                        ...
                        address token = inputs.getAddress();
                        address recipient = inputs.getAddress(0x20);
                        uint160 amountMin = inputs.getUint160(0x40);
                        Payments.sweep(token, map(recipient), amountMin);
                    } else if (command == Commands.TRANSFER) {
                        // equivalent:  abi.decode(inputs, (address, address, uint256))
                        address token = inputs.getAddress();
                        address recipient = inputs.getAddress(0x20);
                        uint256 value = inputs.getUint256(0x40);
                        Payments.pay(token, map(recipient), value);
                    } else if (command == Commands.PAY_PORTION) {
                        // equivalent:  abi.decode(inputs, (address, address, uint256))
                        address token = inputs.getAddress();
                        address recipient = inputs.getAddress(0x20);
                        uint256 bips = inputs.getUint256(0x40);
                    ...
                    if (command == Commands.V2_SWAP_EXACT_IN) {
                        ...
                        address recipient = inputs.getAddress();
                        uint256 amountMin = inputs.getUint256(0x20);
                        ...
                        // equivalent: abi.decode(inputs, (address, address, uint256))
                        address owner = inputs.getAddress();
                        address token = inputs.getAddress(0x20);
                        uint256 minBalance = inputs.getUint256(0x40);
                ...
                if (command < Commands.THIRD_IF_BOUNDARY) {
                    if (command == Commands.SEAPORT_V1_5) {
                        ...
                        uint256 punkId = inputs.getUint256();
                        address recipient = inputs.getAddress(0x20);
                        uint256 value = inputs.getUint256(0x40);
                        ...
                        address owner = inputs.getAddress();
                        address token = inputs.getAddress(0x20);
                        uint256 id = inputs.getUint256(0x40);
                        ...
                        address token = inputs.getAddress(0x20);
                        uint256 id = inputs.getUint256(0x40);
                        uint256 minBalance = inputs.getUint256(0x60);
                    ...
                    if (command == Commands.X2Y2_721) {
                        ...
                        address recipient = inputs.getAddress(0x20);
                        uint256 id = inputs.getUint256(0x40);
                        uint256 amount = inputs.getUint256(0x60);
            ...
            if (command == Commands.SEAPORT_V1_4) {
                ...
                ERC20 token = ERC20(inputs.getAddress());
                RouterImmutables.Spenders spender = RouterImmutables.Spenders(inputs.getUint8(0x20));
    ...
    function callAndTransfer721(
        ...
        address recipient = inputs.getAddress(0x40);
        address token = inputs.getAddress(0x60);
        uint256 id = inputs.getUint256(0x80);
    ...
    function callAndTransfer1155(
        ...
        address token = inputs.getAddress(0x60);
        uint256 id = inputs.getUint256(0x80);
        uint256 amount = inputs.getUint256(0xa0);
        (success, output) = protocol.call{value: value}(data);
