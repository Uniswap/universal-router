...
import { ALICE_ADDRESS, DEADLINE, ETH_ADDRESS } from './shared/constants'
...
const SUDOLETS_PAIR = '0x339e7004372e04b1d59443f0ddc075efd9d80360'
...
describe.only('Sudoswap', () => {
  ...
  describe('Sell a sudolet on sudoswap', () => {
    ...
    beforeEach(async () => {
      ...
      await (
        await router['execute(bytes,bytes[],uint256)'](commands, inputs, DEADLINE, { value: oneEther })
      ).wait()
    })
    ...
    it('sell token ids 80', async () => {
      ...
      planner.addCommand(CommandType.SUDOSWAP_SELL, [calldata, SUDOLETS_ADDRESS, SUDOLETS_PAIR, 80])
      const { commands, inputs } = planner
