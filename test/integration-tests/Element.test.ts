...
describe.only('Element Market polygon', () => {
  ...
  const jsonRpcUrl =  `https://polygon-mainnet.g.alchemy.com/v2/${process.env.ALCHEMY_API_KEY}` // `https://polygon-mainnet.infura.io/v3/${process.env.INFURA_API_KEY}`
  ...
  it('purchases open order', async () => {
    ...
    console.log(hash)
    const hashNonce = await element.callStatic.getHashNonce(EXAMPLE_NFT_SELL_ORDER.maker)
